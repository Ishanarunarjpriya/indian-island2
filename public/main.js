...
function addPalm(x, z, scale = 1) {
  ...
  // Fronds (6 per palm)
  for (let f = 0; f < 6; f++) {
    _pos.set(x, 5.45*scale, z);
    _euler.set(Math.PI/2.35, (f/6)*Math.PI*2, 0); _quat.setFromEuler(_euler);
    _scale.setScalar(scale);
    _m4.compose(_pos, _quat, _scale);
...
function buildPlayerMesh(appearance) {
  ...
  if (app.accessories.includes('glasses')) {
    ...
    const lensL = new THREE.Mesh(new THREE.TorusGeometry(0.095, 0.018, 8, 16), glassMat);
    lensL.position.set(-0.13, 1.6, 0.28); lensL.rotation.y = Math.PI/2; group.add(lensL);
    const lensR = lensL.clone(); lensR.position.x = 0.13; group.add(lensR);
  }
